...
from musedashboard.api_manager import find_genres, find_bpm, find_release_date, find_gain, find_artist_fans, \
    find_album_release_date, find_album_fans
...
def get_times_listening_in_seconds(df_row, time_column: str, duration_column: str) -> float:
...
def compute_listening_time(df: pd.DataFrame, data_column: str) -> pd.DataFrame:
    ...
    df['time_between_listening_seconds'] = df['time_between_listening'].apply(lambda x: x.total_seconds())
    df['listening_time'] = df.apply(
        lambda x: get_times_listening_in_seconds(x, 'time_between_listening_seconds', 'duration'), axis=1)
